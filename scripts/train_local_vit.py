#!/usr/bin/env python
"""
Local training script for Vision Transformer on ~3000 images, 30 epochs.
Runs on CPU.
Estimated time: ~2-2.5 hours on i5-1235U
"""
...
def main():
    ...
    print('=' * 70)
    print('LOCAL TRAINING: Vision Transformer on ~3000 images, 30 epochs')
    print('=' * 70)
    ...
    model = create_vit_model(
        ...
        embed_dim=256,
        encoder_layers=4,  # CPU-optimized
        decoder_layers=4
    )
    ...
    trainer = Trainer(
        ...
        text_preprocessor=text_proc,
        checkpoint_dir='checkpoints/local_3k_vit',
        output_dir='outputs/local_3k_vit'
    )
    ...
    os.makedirs('outputs/local_3k_vit', exist_ok=True)
    with open('outputs/local_3k_vit/training_results.json', 'w') as f:
    ...
    print(f"Model saved to: checkpoints/local_3k_vit/")
    print(f"Results saved to: outputs/local_3k_vit/training_results.json")
